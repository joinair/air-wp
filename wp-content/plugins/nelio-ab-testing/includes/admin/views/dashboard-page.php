		...
		public function do_render() {
			...
			if ( count( $this->experiments ) == 0 ) {
				...
				echo sprintf( '<img class="animated flipInY" src="%s" alt="%s" />',
					...
					__( 'Dashboard Icon', 'nelio-ab-testing' )
				);
				echo '<h2 style="max-width:750px;">';
				printf( '%1$s<br><br><a class="button button-primary" href="%3$s">%2$s</a>',
					__( 'Here you\'ll find relevant information about your running experiments.', 'nelio-ab-testing' ),
					_x( 'Create One Now!', 'create-experiment', 'nelio-ab-testing' ),
					'admin.php?page=nelioab-add-experiment' );
				echo '</h2>';
				echo '</div>';
			}
			else {
				echo '<h2>' . __( 'Running Experiments', 'nelio-ab-testing' ) . '</h2>';
				$this->print_cards();
			}
			echo '</div>'; ?>
			<div id="postbox-container-1" class="postbox-container" style="overflow:hidden;">
				...
				<div class="numbers" style="height:40px;">
					<div class="left" style="float:left; width:58%;">
						<span style="font-weight:bold;"><?php _e( 'AVAILABLE QUOTA', 'nelio-ab-testing' ); ?></span><br>
		...
		public function print_rss() {
			...
			if ( ! is_wp_error( $this->rss ) ) : // Checks that the object is created correctly
				...
				<div id="nelio-rss" class="postbox-container" style="overflow:hidden;">
					<h2><?php _e( 'Latest News', 'nelio-ab-testing' ); ?></h2>
		...
		public function get_beautiful_title( $exp ) {
			...
			switch ( $exp->get_type() ) {
				case NelioABExperiment::PAGE_ALT_EXP:
					try {
						...
						if ( $page_on_front == $aux[0]['id'] )
							$img = sprintf( $img, 'landing-page', __( 'Landing Page', 'nelio-ab-testing' ) );
						else
							$img = sprintf( $img, 'page', __( 'Page', 'nelio-ab-testing' ) );
					}
					catch ( Exception $e ) {
						$img = sprintf( $img, 'page', __( 'Page', 'nelio-ab-testing' ) );
					}
				...
				case NelioABExperiment::POST_ALT_EXP:
					$img = sprintf( $img, 'post', __( 'Post', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::CPT_ALT_EXP:
					$img = sprintf( $img, 'cpt', __( 'Post', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::HEADLINE_ALT_EXP:
					$img = sprintf( $img, 'title', __( 'Headline', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::WC_PRODUCT_SUMMARY_ALT_EXP:
					$img = sprintf( $img, 'wc-product-summary', __( 'WooCommerce Product Summary', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::THEME_ALT_EXP:
					$img = sprintf( $img, 'theme', __( 'Theme', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::CSS_ALT_EXP:
					$img = sprintf( $img, 'css', __( 'CSS', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::HEATMAP_EXP:
					$img = sprintf( $img, 'heatmap', __( 'Heatmap', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::WIDGET_ALT_EXP:
					$img = sprintf( $img, 'widget', __( 'Widget', 'nelio-ab-testing' ) );
					break;
				case NelioABExperiment::MENU_ALT_EXP:
					$img = sprintf( $img, 'menu', __( 'Menu', 'nelio-ab-testing' ) );
					break;
				default:
					$img = '';
			}
			...
			<div class="row padding-top">
				<div class="col col-4">
					<div class="row data padding-left">
						...
						<span class="label"><?php _e( 'Page Views', 'nelio-ab-testing' ); ?></span>
					</div>
					<div class="row data padding-left">
						<span class="value"><?php echo count( $exp->get_alternative_info() ); ?></span>
						<span class="label"><?php _e( 'Alternatives', 'nelio-ab-testing' ); ?></span>
					</div>
				</div>
				<div class="col col-4">
					<div class="row data">
						...
						<span class="label"><?php _e( 'Original Version\'s Conversion Rate', 'nelio-ab-testing' ); ?></span>
					</div>
					<div class="row data">
						<?php
						...
						?>
						<span class="value"><?php echo $val; ?></span>
						<span class="label"><?php _e( 'Best Alternative\'s Conversion Rate', 'nelio-ab-testing' ); ?></span>
					</div>
				</div>
				...
				<script>jQuery(document).ready(function() {
					var aux = setTimeout( function() {
						drawGraphic('<?php echo $graphic_id; ?>',
							[<?php echo $values; ?>],
							"<?php echo esc_html( __( 'Conversions', 'nelio-ab-testing' ) ); ?>",
							"<?php echo $color; ?>");
			...
			<div class="row padding-top">
				<div class="col col-6">
					<div class="row data phone padding-left">
						...
						<span class="label"><?php _e( 'Views on Phone', 'nelio-ab-testing' ); ?></span>
					</div>
					<div class="row data tablet padding-left">
						<span class="value"><?php echo $hm['tablet']; ?></span>
						<span class="label"><?php _e( 'Views on Tablet', 'nelio-ab-testing' ); ?></span>
					</div>
				</div>
				<div class="col col-6">
					<div class="row data desktop">
						<span class="value"><?php echo $hm['desktop']; ?></span>
						<span class="label"><?php _e( 'Views on Desktop', 'nelio-ab-testing' ); ?></span>
					</div>
					<div class="row data hd">
						<span class="value"><?php echo $hm['hd']; ?></span>
						<span class="label"><?php _e( 'Views on Large Screens', 'nelio-ab-testing' ); ?></span>
					</div>
				</div>
			</div>
			<?php
