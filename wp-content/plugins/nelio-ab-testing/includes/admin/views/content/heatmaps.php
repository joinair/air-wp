	...
	if ( isset( $_POST['load_from_appengine'] ) ) {
		try {
			...
			echo sprintf( '<img src="%s" alt="%s" style="margin-top:50px;"/>',
				...
				__( 'Funny image to graphically notify of an error.', 'nelio-ab-testing' )
			);
			?>
				...
				window.onbeforeunload = function() {
					if ( nelioab__framekiller ) {
						return "<?php echo str_replace( '"', '\\"', str_replace( '\\', '\\\\',
								__( 'Apparently, there\'s a script that\'s trying to overwrite the location in the address bar and take you somewhere else. Please, in order to see the Heatmaps and Clickmaps of this experiment, make sure you stay in this page and don\'t leave.', 'nelio-ab-testing' )
							) ); ?>";
					}
				};
			</script>
		...
		<script>
			...
			NelioABHeatmapLabels.hm.view      = "<?php echo esc_html( __( 'View Heatmap', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.phone     = "<?php echo esc_html( __( 'Smartphone', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.phoneNo   = "<?php echo esc_html( __( 'Smartphone (no Heatmap available)', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.tablet    = "<?php echo esc_html( __( 'Tablet', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.tabletNo  = "<?php echo esc_html( __( 'Tablet (no Heatmap available)', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.desktop   = "<?php echo esc_html( __( 'Laptop Monitor', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.desktopNo = "<?php echo esc_html( __( 'Laptop Monitor (no Heatmap available)', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.hd        = "<?php echo esc_html( __( 'Regular Desktop Monitor', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.hm.hdNo      = "<?php echo esc_html( __( 'Regular Desktop Monitor (no Heatmap available)', 'nelio-ab-testing' ) ); ?>";

			NelioABHeatmapLabels.cm.view      = "<?php echo esc_html( __( 'View Clickmap', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.phone     = "<?php echo esc_html( __( 'Smartphone', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.phoneNo   = "<?php echo esc_html( __( 'Smartphone (no Clickmap available)', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.tablet    = "<?php echo esc_html( __( 'Tablet', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.tabletNo  = "<?php echo esc_html( __( 'Tablet (no Clickmap available)', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.desktop   = "<?php echo esc_html( __( 'Laptop Monitor', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.desktopNo = "<?php echo esc_html( __( 'Laptop Monitor (no Clickmap available)', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.hd        = "<?php echo esc_html( __( 'Regular Desktop Monitor', 'nelio-ab-testing' ) ); ?>";
			NelioABHeatmapLabels.cm.hdNo      = "<?php echo esc_html( __( 'Regular Desktop Monitor (no Clickmap available)', 'nelio-ab-testing' ) ); ?>";
...
<head>
	<title><?php _e( 'Nelio AB Testing &mdash; Heatmaps Viewer', 'nelio-ab-testing' ); ?></title>
...
<body>
	...
	<div id="toolbar" data-resizer="basic">
		...
		<ul id="hm-additional-controls">
			<li style="color:white;font-size:12px;font-weight:bold;" id="visitors-count"><?php printf( __( 'Heatmap built using data from %s page views', 'nelio-ab-testing' ), 0 ); ?></li>
			<?php
			if ( $show_back_link ) {
				$link = admin_url( 'admin.php?page=nelioab-experiments&action=progress&id=%1$s&exp_type=%2$s' );
				?>
				<li>|</li>
				<li><a style="font-size:12px;" href="<?php printf( $link, $_GET['id'], $_GET['exp_type'] ); ?>"><?php echo __( 'Back', 'nelio-ab-testing' ); ?></a></li>
			<?php
			} ?>
			<li>|</li>
		...
		<ul id="hm-dropdown-controls" style="display:none;">
			...
			<li><a id="view-clicks" style="font-size:12px;"><?php echo __( 'View Clickmap', 'nelio-ab-testing' ); ?></a></li>
			<li><a style="font-size:12px;" href="<?php echo admin_url( 'admin.php?page=nelioab-experiments' ); ?>"><?php echo __( 'List of experiments', 'nelio-ab-testing' ); ?></a></li>
		</ul>
	</div>
