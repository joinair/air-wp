...
if ( !class_exists( 'NelioABAccountPage' ) ) {
	...
	class NelioABAccountPage extends NelioABAdminAjaxPage {
		...
		protected function do_render() { ?>
			<form id="nelioab_account_form" method="post">
				...
				$fields = array(
						array (
							'label'     => __( 'E-Mail', 'nelio-ab-testing' ),
							'id'        => 'settings_email',
							'callback'  => array( &$this, 'print_email_field' ),
							'mandatory' => true ),
						array (
							'label'     => __( 'Registration Number', 'nelio-ab-testing' ),
							'id'        => 'settings_reg_num',
				...
				if ( !NelioABAccountSettings::is_using_free_trial() ) {
					array_push( $fields, array (
						'label'     => __( 'I have read and accept the <a href="https://nelioabtesting.com/terms-and-conditions/?plugin=tac" target="_blank">Terms and Conditions</a> of this service.', 'nelio-ab-testing' ),
						'id'        => 'settings_tac',
						'mandatory' => true,
						'checkbox'  => true,
						'checked'   => $this->tac,
			...
			if ( $print_plans ) {
				$this->set_message(
					__( 'Haven\'t you subscribed to any of our plans? <b><a href="https://nelioabtesting.com/subscription-plans/?plugin=account-message" target="_blank">Check them out and choose the one that best fits you</a></b>!', 'nelio-ab-testing' ) );
			}
			?>
			<h2 style="margin-bottom:0px;padding-bottom:0px;"><?php
				...
				echo __( 'Account Information', 'nelio-ab-testing' ) . '&nbsp;&nbsp;' . $status_title;
			...
			if ( ! $this->tac || ! $is_status_defined ||
					! $this->is_email_valid || ! $this->is_reg_num_valid ) {
				echo '<p>';
				_e( 'Please, fill in all required fields in order to view your account details and use our service.', 'nelio-ab-testing' );
				echo '</p>';
			}
			else { ?>
				...
				<?php if ( !$this->user_info['agency'] && !empty( $this->user_info['firstname'] ) ) { ?>
					<p style="margin-top:0em;margin-left:3em;"><?php
						printf( __( 'Hi %s!', 'nelio-ab-testing' ),  $this->user_info['firstname'] );
					?></p>
				<?php } ?>

				<p style="margin-top:0em;margin-left:3em;"><?php
					if ( !isset( $this->user_info['subscription_url'] ) ) {
						_e( 'No subscription information available.', 'nelio-ab-testing' );
					}
						...
						if ( $this->user_info['subscription_plan'] == 0 ) {
							printf( '%s<br /><a href="%s">%s</a>',
								...
								__( 'Subscribe now and continue using our service!', 'nelio-ab-testing' ) );
						}
						else {
							if ( ! $this->user_info['agency'] ) {
								...
								if ( $this->is_account_active ) {
									...
									if ( $this->user_info['subscription_plan'] == NelioABAccountSettings::BASIC_SUBSCRIPTION_PLAN ) {
										...
										printf(
											'<a href="%2$s">%1$s</a>',
											__( 'Upgrade to our Professional Plan.', 'nelio-ab-testing' ),
									...
									if ( $this->user_info['subscription_plan'] == NelioABAccountSettings::PROFESSIONAL_SUBSCRIPTION_PLAN ) {
										...
										printf(
											'<a href="%2$s">%1$s</a>',
											__( 'Upgrade to our Enterprise Plan.', 'nelio-ab-testing' ),
											'mailto:support@neliosoftware.com?' .
				...
				if ( $this->is_account_active ) {
					...
					if ( !NelioABAccountSettings::is_using_free_trial() && isset( $this->user_info['quota'] ) ) { ?>
						...
						<b><?php _e( 'Available Quota:', 'nelio-ab-testing' ); ?></b>
						<?php
				...
				if ( !NelioABAccountSettings::is_using_free_trial() && $this->is_account_active &&
					...
					if ( $this->error_retrieving_registered_sites ) { ?>
						<h3><?php _e( 'Active Sites', 'nelio-ab-testing' ); ?></h3><?php


						?><p style="margin-top:0em;margin-left:3em;"><?php
							echo __( 'There was an error while retrieving the list of active sites related to this account.', 'nelio-ab-testing' );

						if ( NelioABAccountSettings::has_a_configured_site() ) {
							echo ' ';
							echo __( 'Nonetheless, this site is active and ready to be tested.', 'nelio-ab-testing' );
						}
						...
						switch( $this->current_site_status ) {
							...
							default:
								?> <h3><?php _e( 'This Site is Not Active', 'nelio-ab-testing' ); ?></h3> <?php
					...
					(function($) {
						...
						$dialog.dialog({
							...
							closeOnEscape : true,
							width         : 400,
							title         : "<?php echo esc_html( __('About Your Site', 'nelio-ab-testing' ) ); ?>",
							buttons: [
								{
									text: "<?php echo esc_html( __( 'Skip and Activate', 'nelio-ab-testing' ) ); ?>",
									click: function() {
										$(this).dialog('close');
								...
								{
									text: "<?php echo esc_html( __( 'Activate', 'nelio-ab-testing' ) ); ?>",
		...
		public function print_dialog_content() { ?>
			<p><?php
				_e( 'In order to offer relevant and valuable suggestions, we need to know a little bit more about your site. Please, answer the following two questions:', 'nelio-ab-testing' );
			?></p>
			<p><strong><?php
				_e( 'How would you describe your site?', 'nelio-ab-testing' );
			?></strong></p>
			...
			<p><strong><?php
				_e( 'What is it focused on?', 'nelio-ab-testing' );
			?></strong></p>
		...
		public function print_reg_num_field() { ?>
			...
			if ( $this->is_email_valid) {
				if ( $this->is_reg_num_valid ) { ?>
					<span style="color:#00AA00; font-weight:bold;"><?php _e( 'OK', 'nelio-ab-testing' ); ?></span><?php
				}
				else { ?>
					<span style="color:red; font-weight:bold;"><?php _e( 'INVALID', 'nelio-ab-testing' ); ?></span><?php
				}
			}
		}
		...
		private function print_site_should_be_registered() {
			...
			echo sprintf( "<p $style>%s <strong><a href=\"#\" id=\"register-site-button\">%s</a></strong></p>\n",
				__( 'This site is not yet active in your account.', 'nelio-ab-testing' ),
				__( 'Activate it now!', 'nelio-ab-testing' ) );
		}

		private function print_site_cannot_be_registered() {
			$style = 'style="' . $this->p_style . 'margin-left:3em;font-size:120%%;line-height:1.3em;"';
			echo sprintf( "<p $style>%s</p><p $style>%s</p>\n",
				__( 'It looks like this site is not connected to you account. In order to use our service in this site, you have to activate it. Unfortunately, you already reached the maximum number of sites allowed by your current subscription plan (see the table below).', 'nelio-ab-testing' ),
	...
	class NelioABRegisteredSitesTable extends NelioABAdminTable {
		...
		private function make_this_site( $site ) {
			...
			$name .= ' <strong style="font-variant:small-caps;">(' . __( 'this site', 'nelio-ab-testing' ) . ')</strong> ';
			...
			if ( $is_live ) {
				...
				$actions = $this->row_actions( array(
						'delete' => '<a href="' . $link . '">' . __( 'Deactivate', 'nelio-ab-testing' ) . '</a>'
					) );
			}
			else {
				$link = 'javascript:' .
					...
					'jQuery(\'#nelioab_registration_form\').submit();';
				$actions = $this->row_actions( array(
						'delete' => '<a href="' . $link . '">' . __( 'Unlink Site', 'nelio-ab-testing' ) . '</a>'
					) );
			}
			...
			if ( !$is_live ) {
				$careful = sprintf(
						...
						$reg_title,
						__( '* The site was activated using a different URL. If you want to activate this site using its own URL, unlink it first.', 'nelio-ab-testing' )
					);
			}
