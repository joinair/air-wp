	...
	class NelioABInvalidConfigPage extends NelioABAdminAjaxPage {
		...
		protected function do_render() {
			...
			if ( NelioABAccountSettings::can_free_trial_be_started() ) {
				...
				echo '<h2>' . __( 'Welcome!', 'nelio-ab-testing' ) . '</h2>';
				...
				printf( "<p class=\"nelio-admin-explanation\"><strong>%s</strong></p>\n",
						__( 'Let\'s get started!', 'nelio-ab-testing' )
					);

				$account_url = admin_url( 'admin.php?page=nelioab-account&nabmode=my-account' );
				$my_account_button = $this->make_button(
					__( 'Use Nelio Account', 'nelio-ab-testing' ), $account_url, false );
				$free_trial_button = $this->make_button(
					__( 'Start Free Trial', 'nelio-ab-testing' ), '#', true );

				$tac_text = sprintf(
					__( 'By starting the free trial you agree to be legally bound by these <a href="%s" target="_blank">terms</a>.', 'nelio-ab-testing' ),
				...
				$account_url = admin_url( 'admin.php?page=nelioab-account&nabmode=my-account' );
				$my_account_button = $this->make_button(
					__( 'Use Nelio Account', 'nelio-ab-testing' ), $account_url, true );
				$free_trial_button = '';

			} else {

				echo '<h2>' . __( 'Setup', 'nelio-ab-testing' ) . '</h2>';
				printf( "<p class=\"nelio-admin-explanation\">%s</p>\n",
				...
				printf( "<p class=\"nelio-admin-explanation\"><strong>%s</strong></p>\n",
						__( 'Activate this site in your account.', 'nelio-ab-testing' )
					);

				$account_url = admin_url( 'admin.php?page=nelioab-account&nabmode=my-account' );
				$my_account_button = $this->make_button(
					__( 'Open My Account', 'nelio-ab-testing' ), $account_url, true );
				$free_trial_button = '';
			}
