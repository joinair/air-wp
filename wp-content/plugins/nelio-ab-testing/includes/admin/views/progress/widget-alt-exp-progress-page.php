...
if ( !class_exists( 'NelioABWidgetAltExpProgressPage' ) ) {
	...
	class NelioABWidgetAltExpProgressPage extends NelioABAltExpProgressPage {
		...
		protected function print_experiment_details_title() {
			_e( 'Details of the Widget Experiment', 'nelio-ab-testing' );
		}
		...
		protected function print_winner_info() {
			...
			if ( $exp->get_status() == NelioABExperiment::STATUS_RUNNING ) {
				if ( $the_winner == 0 )
					echo '<p><b>' . __( 'Right now, no alternative Widget set is helping to improve your site.', 'nelio-ab-testing' ) . '</b></p>';
				if ( $the_winner > 0 )
					echo '<p><b>' . sprintf( __( 'Right now, the alternative %s is better than none to improve your site.', 'nelio-ab-testing' ), $the_winner ) . '</b></p>';
			}
			else {
				if ( $the_winner == 0 )
					echo '<p><b>' . __( 'No alternative Widget set helped to improve your site.', 'nelio-ab-testing' ) . '</b></p>';
				if ( $the_winner > 0 )
					echo '<p><b>' . sprintf( __( 'The alternative Widget set %s was better than the original set.', 'nelio-ab-testing' ), $the_winner ) . '</b></p>';
			}
		}
		...
		protected function print_the_original_alternative() {
			...
			$ori_label       = __( 'Original Version', 'nelio-ab-testing' );
			$ori_name        = __( 'Original', 'nelio-ab-testing' );

			$conversions_label      = __( 'Conversions', 'nelio-ab-testing' );
			$pageviews_label        = __( 'Pageviews', 'nelio-ab-testing' );
			$conversion_views_label = __( 'Conversions / Views', 'nelio-ab-testing' );
			$conversion_rate_label  = __( 'Conversion Rate', 'nelio-ab-testing' );

			$name = $this->trunk( $this->get_original_name() );

			$original = __( 'This is the original version', 'nelio-ab-testing' );
						...
						if ( $winner ) {
							if ( NelioABExperimentsManager::current_user_can( $exp, 'apply' ) ) {
								$aux = sprintf( '<a class="apply-link button button-primary" href="javascript:nelioab_confirm_overwriting(%1$s,\'apply-ori-and-clean\');">%2$s %3$s</a>',
									$exp->get_originals_id(), $img, __( 'Apply and Clean', 'nelio-ab-testing' ) );
							} else {
								$aux = sprintf( '<a class="apply-link button button-primary disabled" href="#">%s</a>', __( 'Apply and Clean', 'nelio-ab-testing' ) );
							}
						} else {
							if ( NelioABExperimentsManager::current_user_can( $exp, 'apply' ) ) {
								...
								$aux = sprintf( '<a class="apply-link button disabled" href="#">%s</a>', __( 'Apply and Clean', 'nelio-ab-testing' ) );
							}
						}
...
HTML;
		...
		protected function print_the_real_alternatives() {
			...
			$conversion_views_label = __( 'Conversions / Views', 'nelio-ab-testing' );
			$conversion_rate_label  = __( 'Conversion Rate', 'nelio-ab-testing' );
			$alternative_label      = __( 'Alternative', 'nelio-ab-testing' );
							...
							if ( $winner ) {
								if ( NelioABExperimentsManager::current_user_can( $exp, 'apply' ) ) {
									$auxl = sprintf( '<a class="apply-link button button-primary" href="javascript:nelioab_confirm_overwriting(%1$s,\'apply-alt-and-clean\');">%2$s %3$s</a>',
										$alt->get_id(), $img, __( 'Apply and Clean', 'nelio-ab-testing' ) );
								} else {
									$auxl = sprintf( '<a class="apply-link button button-primary disabled" href="#">%s</a>', __( 'Apply and Clean', 'nelio-ab-testing' ) );
								}
							} else {
								if ( NelioABExperimentsManager::current_user_can( $exp, 'apply' ) ) {
									...
									$auxl = sprintf( '<a class="apply-link button disabled" href="#">%s</a>', __( 'Apply and Clean', 'nelio-ab-testing' ) );
								}
							}
...
HTML;
		...
		protected function get_labels_for_conversion_rate_js() {
			...
			$labels['detail']   = __( '<b>{0}</b><br />Conversions: {1}%', 'nelio-ab-testing' );
			return $labels;
		}
		...
		protected function get_labels_for_improvement_factor_js() {
			...
			$labels['column']   = __( '{0}%', 'nelio-ab-testing' );
			$labels['detail']   = __( '<b>{0}</b><br />{1}% improvement', 'nelio-ab-testing' );
			return $labels;
		}

		protected function get_labels_for_visitors_js() {
			$labels = array();
			$labels['title']       = __( 'Page Views and Conversions', 'nelio-ab-testing' );
			$labels['subtitle']    = __( 'for default and alternative Widget sets', 'nelio-ab-testing' );
			$labels['xaxis']       = __( 'Alternatives', 'nelio-ab-testing' );
			$labels['detail']      = __( 'Number of {series.name}: <b>{point.y}</b>', 'nelio-ab-testing' );
