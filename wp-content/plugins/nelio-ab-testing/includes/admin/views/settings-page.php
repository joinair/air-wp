	...
	class NelioABSettingsPage extends NelioABAdminPage {
		...
		public function __construct( $title ) {
			...
			if ( count( $ae_sync_errors ) > 0 ) {
				$msg = __( 'There was a problem while updating some of your options. The following fields could not be properly updated:</p>%s<p>Please, try it again in a few moments.', 'nelio-ab-testing' );

				$errors = '<ul>';
				if ( in_array( 'algorithm', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Algorithm', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'make_site_consistent', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Site-wide Consistency', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'expl_ratio', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Exploitation or Exploration (using Greedy Algorithm)', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'get_params_visibility', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'A/B GET Params', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'hm_tracking_mode', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Heatmap Tracking', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'user_split', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Test Mode', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'ori_perc', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Original Percentage (using Prioritize Original Algorithm)', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'perc_of_tested_users', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Num. of Tested Users', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'quota_limit_per_exp', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Quota Limit', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'notification_email', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Notification E-Mail', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'notifications', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'Notifications', 'nelio-ab-testing' ) . '</li>';
				if ( in_array( 'call_tracking_key', $ae_sync_errors ) )
					$errors .= '<li>- ' . __( 'The Customer ID of Your Call Tracking Service', 'nelio-ab-testing' ) . '</li>';
				$errors .= '</ul>';
				if ( '<ul></ul>' == $errors )
					$errors = '';
				...
				<form method="post" action="options.php">
					<h3 id="settings-tabs" class="nav-tab-wrapper" style="margin:0em;padding:0em;padding-left:2em;margin-bottom:2em;"><?php
						...
						printf( $tab, 'basic', 'nav-tab-active',
							__( 'Basic', 'nelio-ab-testing' ) );

						printf( $tab, 'pro', '',
							__( 'Advanced', 'nelio-ab-testing' ) );
						...
						jQuery(document).ready( function() {
							...
							jQuery('#dialog-content').html("<?php
								echo str_replace( '"', '\\"', str_replace( '\\', '\\\\',
									__( 'This operation will set all Settings to their default values. Do you want to continue?', 'nelio-ab-testing' ) ) );
							?>");
		...
		public static function print_basic_section_efficiency() {
			...
			echo '<h3>' . __( 'Plugin Behavior', 'nelio-ab-testing' ) . '</h3>';
		}

		public static function print_basic_section_ms() {
			echo '<br><br>';
			echo '<h3>' . __( 'Multi-Site Settings', 'nelio-ab-testing' ) . '</h3>';
		}

		public static function print_basic_section_ca() {
			echo '<br><br>';
			echo '<h3>' . __( 'Conversion Actions', 'nelio-ab-testing' ) . '</h3>';
		}

		public static function print_basic_section_ui() {
			echo '<br><br>';
			echo '<h3>' . __( 'User Interface', 'nelio-ab-testing' ) . '</h3>';
		}

		public static function print_basic_section_misc() {
			echo '<br><br>';
			echo '<h3>' . __( 'Miscellaneous', 'nelio-ab-testing' ) . '</h3>';
		}
			...
			?>
				<option value='<?php
					echo NelioABSettings::ALGORITHM_PURE_RANDOM; ?>'><?php
						_e( 'Default - Pure Random', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					...
					if ( NelioABSettings::get_algorithm() == NelioABSettings::ALGORITHM_PRIORITIZE_ORIGINAL )
						...
						_e( 'Prioritize Original Version', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					echo NelioABSettings::ALGORITHM_GREEDY; ?>' <?php
					if ( NelioABSettings::get_algorithm() == NelioABSettings::ALGORITHM_GREEDY )
						echo ' selected="selected"'; ?>><?php
						_e( 'Prioritize Winner (Greedy)', 'nelio-ab-testing' ); ?></option>
			</select>
			<br><div class="the-descr" style="display:none;"><span <?php echo self::get_pro_details( 'description' ); ?>><?php
				_e( 'Nelio A/B Testing implements different algorithms for deciding which alternative should see each of your visitors.', 'nelio-ab-testing' );
		...
		public static function print_quota_limit_per_experiment_field() {
			...
			?>
				<option value='-1'><?php _e( 'Unlimited', 'nelio-ab-testing' ); ?></option>
			<?php
			...
			foreach ( $options as $v ) {
				printf( '<option value="%2$s" %3$s>%1$s</option>',
					sprintf( __( '%s page views', 'nelio-ab-testing' ), number_format_i18n( $v ) ),
					strval( $v ),
			...
			<br><div class="the-descr" style="display:none;"><span <?php echo self::get_pro_details( 'description' ); ?>><?php
				_e( 'This setting will automatically stop the experiment as soon as the results of an experiment are updated and the number of page views is equal to (or a little bit greater than) the limit you set. Don\'t waste your quota anymore!', 'nelio-ab-testing' );
			?></span></div>
			<?php
		}
		...
		public static function print_headlines_quota_mode_field() {
			...
			?>
				<option value='<?php
					...
					?>'><?php _e( 'All Pages Are Relevant for Headline Tracking', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					...
					if ( NelioABSettings::get_headlines_quota_mode() == NelioABSettings::HEADLINES_QUOTA_MODE_ON_FRONT_PAGE )
						echo ' selected="selected"';
				?>><?php _e( 'Only the Front Page Is Relevant for Headline Tracking', 'nelio-ab-testing' ); ?></option>
			</select>
			<br><div class="the-descr" style="display:none;"><span <?php echo self::get_pro_details( 'description' ); ?>><?php
		...
		public static function print_site_consistency_field() {
			...
			?>
				<option value='1'><?php _e( 'Force Consistency All Along the Site', 'nelio-ab-testing' ); ?></option>
				<option value='0'<?php
					if ( !NelioABSettings::make_site_consistent() )
						echo ' selected="selected"';
				?>><?php _e( 'Load Alternative Content for Tested Elements Only', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				_e( 'When a page or post experiment is created, alternative titles, contents, featured images, and excerpts may be defined. Where and when is this alternative content loaded?', 'nelio-ab-testing' );
				echo '<ul style="list-style-type:disc;margin-left:3em;">';
		...
		public static function print_colorblindness_field() {
			...
			?>
				<option value='0'><?php _e( 'Regular Palette', 'nelio-ab-testing' ); ?></option>
				<option value='1'<?php
					if ( NelioABSettings::use_colorblind_palette() )
						echo ' selected="selected"';
				?>><?php _e( 'Colorblind Palette', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				_e( 'If you enable the Colorblind Palette, Nelio A/B Testing icons will not be simple, plain colors, but they will include visual clues.', 'nelio-ab-testing' );
			?></span></div><?php
		}
			...
			?>
				<option value='<?php
					echo NelioABSettings::MENU_LOCATION_DASHBOARD; ?>'><?php
						_e( 'Top of the Dashboard', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
				...
				<option value='<?php
					...
					if ( NelioABSettings::get_menu_location() == NelioABSettings::MENU_LOCATION_LAST_BLOCK )
						echo ' selected="selected"'; ?>><?php
						_e( 'First Option in Last Block', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					echo NelioABSettings::MENU_LOCATION_END; ?>' <?php
					if ( NelioABSettings::get_menu_location() == NelioABSettings::MENU_LOCATION_END )
						echo ' selected="selected"'; ?>><?php
						_e( 'Latest Option', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				_e( 'Set the location of «Nelio A/B Testing» in the left pane of the Dashboard.', 'nelio-ab-testing' );
			?></span></div><?php
		}
			...
			?>
				<option value='<?php
					...
					if ( NelioABSettings::get_call_tracking_service() == NelioABSettings::CALL_TRACKING_NONE )
						echo ' selected="selected"'; ?>><?php
						_e( 'None', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					...
					if ( NelioABSettings::get_call_tracking_service() == NelioABSettings::CALL_TRACKING_RESPONSETAP )
						echo ' selected="selected"'; ?>><?php
						_e( 'ResponseTap', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				_e( 'Nelio A/B Testing is compatible with some call tracking services. If one of your visitors calls you using a phone number provided by one of those services, you may track that call and count it as a conversion.', 'nelio-ab-testing' );
			?></span></div>
			<div id="call_tracking_service_extra" style="display:none;"><?php
				echo '<br><p>' . __( 'Please fill in the following information to make ResponseTap conversion actions available.', 'nelio-ab-testing' ) . '</p>';
				printf(
					...
					__( 'Customer ID', 'nelio-ab-testing' )
				);
			?></div>
			<script type="text/javascript">
			(function( $ ) {
		...
		public static function print_menu_in_admin_bar_field() {
			...
			?>
				<option value='1'><?php _e( 'Show «Nelio A/B Testing» Menu in Admin Bar', 'nelio-ab-testing' ); ?></option>
				<option value='0'<?php
					if ( !NelioABSettings::is_menu_enabled_for_admin_bar() )
						echo ' selected="selected"';
				?>><?php _e( 'Hide «Nelio A/B Testing» Menu from Admin Bar', 'nelio-ab-testing' ); ?></option>
			</select>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				_e( 'When browsing your site as an admin, the plugin adds a «Nelio A/B Testing» entry in the top bar, which includes some shortcuts to common A/B Testing functions.', 'nelio-ab-testing' );
			?></span></div><?php
		}
		...
		public static function print_notification_email_field() {
			...
			printf(
				...
				sprintf( __( 'Default: %s', 'nelio-ab-testing' ), esc_html( NelioABAccountSettings::get_email() ) )
			);
			?>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				printf(
		...
		public static function print_notifications_field() {
			...
			printf( $cb, 'notify_exp_finalization',
					__( 'Notify me when an experiment is automatically stopped.', 'nelio-ab-testing' ),
		...
		public static function print_show_finished_experiments_field() {
			...
			?>
				<option value='<?php
					...
					?>'><?php _e( 'Hide Finished Experiments', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					...
					if ( NelioABSettings::FINISHED_EXPERIMENTS_SHOW_RECENT == NelioABSettings::show_finished_experiments() )
						echo ' selected="selected"';
				?>><?php _e( 'Show Recently Finished Experiments', 'nelio-ab-testing' ); ?></option>
				<option
					...
					if ( NelioABSettings::FINISHED_EXPERIMENTS_SHOW_ALL == NelioABSettings::show_finished_experiments() )
						echo ' selected="selected"';
				?>><?php _e( 'Show All Finished Experiments', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
				_e( 'By default, the list of experiments in our plugin shows all the experiments you defined. You may filter the experiments by status. When viewing all experiments, you can decide whether <em>finished</em> experiments should be visible or not.', 'nelio-ab-testing' );
			?></span></div><?php
		}
		...
		public static function print_min_confidence_for_significance_field() {
			...
			<script type="text/javascript">
				jQuery("#<?php echo $field_name; ?>").on("input change", function() {
					var str = "<?php
						$str = __( 'Minimum confidence value is set to <strong>{value}%</strong> (at least 95% is recommended).', 'nelio-ab-testing' );
						$str = str_replace( '"', '\\"', $str );
						echo $str;
					?>";
			...
			<div class="the-descr" style="display:none;"><span <?php echo self::get_pro_details( 'description' ); ?>><?php
				_e( 'The confidence value tells you how "trustable" is the fact that one alternative is better than the original. Changing this value will modify the visual clues in Nelio\'s Dashboard and Progress of the Experiment (lower confidence values means that the "green light" appears sooner).', 'nelio-ab-testing' );
			?></span></div>
			<?php
		}
		...
		public static function print_perc_of_tested_users_field() {
			...
			<script type="text/javascript">
				jQuery("#<?php echo $field_name; ?>").on("input change", function() {
					var str = "<?php
						$str = __( '<strong>{value}%</strong> of the users that access your site will participate in the running experiments.', 'nelio-ab-testing' );
						$str = str_replace( '"', '\\"', $str );
						echo $str;
					?>";
					var value = jQuery(this).attr('value');
			...
			<div class="the-descr" style="display:none;"><span <?php echo self::get_pro_details( 'description' ); ?>><?php
				_e( 'When a user accesses your website she may participate in your running experiments. This setting defines how likely it is for a visitor to be part of your tests.', 'nelio-ab-testing' );
			?></span></div>
			<?php
		}
		...
		public static function print_ori_perc_field() {
			...
			<script type="text/javascript">
				...
				jQuery("#<?php echo $field_name; ?>").on("input change", function() {
					var str = "<?php
						$str = __( '<strong>{value}%</strong> of your visitors will see the original version of the experiment.<br>The rest of the users will see the other alternatives.', 'nelio-ab-testing' );
						$str = str_replace( '"', '\\"', $str );
						echo $str;
					?>";
					var value = jQuery(this).attr('value');
		...
		public static function print_expl_ratio_field() {
			...
			<script type="text/javascript">
				...
				jQuery("#<?php echo $field_name; ?>").on("input change", function() {
					var str = "<?php
						$str = __( '<strong>{value}%</strong> of your visitors will see the winning alternative.<br>The rest of the users will see the other alternatives.', 'nelio-ab-testing' );
						$str = str_replace( '"', '\\"', $str );
						echo $str;
					?>";
					var value = jQuery(this).attr('value');
		...
		public static function print_heatmap_tracking_mode_field() {
			...
			?>
				<option value='<?php
					...
					?>'><?php _e( 'High Accuracy - Use All HTML Elements', 'nelio-ab-testing' ); ?></option>
				<option value='<?php
					...
					if ( NelioABSettings::get_heatmap_tracking_mode() == NelioABSettings::HTML_BASED_HEATMAP_TRACKING )
						echo ' selected="selected"';
				?>><?php _e( 'Tolerance to Random IDs - Use Body Tag', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
			...
			?>
				...
				<option value='<?php echo $val; ?>'<?php
					if ( NelioABSettings::get_plugin_available_to() == $val )
						echo ' selected="selected"';
				?>><?php
					_e( 'Super Admins and Site Admins', 'nelio-ab-testing' );
				?></option>
				...
				<option value='<?php echo $val; ?>'<?php
					if ( NelioABSettings::get_plugin_available_to() == $val )
						echo ' selected="selected"';
				?>><?php
					_e( 'Super Admins Only', 'nelio-ab-testing' );
				?></option>
			</select>
			<?php
		}
		...
		public static function print_on_blank_field() {
			...
			?>
				<option value='1'><?php
					_e( 'Force New Tab (add «target="_blank"» attribute)', 'nelio-ab-testing' ); ?></option>
				<option value='0'<?php
					if ( !NelioABSettings::use_outwards_navigations_blank() )
						echo ' selected="selected"';
				?>><?php _e( 'Do Not Force New Tab', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
		...
		public static function print_user_split_field() {
			...
			?>
				...
				<option value='<?php echo $val ?>'><?php
					_e( 'Group Experiments and Divide Visitors', 'nelio-ab-testing' ); ?></option>
				?></option>
		...
		public static function print_get_params_visibility_field() {
			...
			?>
				...
				<option value='<?php echo $val ?>'><?php
					_e( 'Show All A/B Testing Parameters (including context)', 'nelio-ab-testing' );
				?></option>
				...
				<option value='<?php echo $val; ?>'<?php
					if ( NelioABSettings::get_params_visibility() == $val )
						echo ' selected="selected"';
				?>><?php
					_e( 'Show This Page A/B Testing Parameter (hide context)', 'nelio-ab-testing' );
				?></option>
				...
				<option value='<?php echo $val; ?>'<?php
					if ( NelioABSettings::get_params_visibility() == $val )
						echo ' selected="selected"';
				?>><?php
					_e( 'Hide All A/B Testing Parameters', 'nelio-ab-testing' );
				?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
			_e( 'Nelio A/B Testing loads alternative content by adding a few parameters in your URL. In principle, this parameters are visible to your visitors, but you can decide whether they should be hidden or not and, if they should, which ones.', 'nelio-ab-testing' );
			echo '<ul style="list-style-type:disc;margin-left:3em;">';
		...
		public static function print_theme_landing_page_field() {
			...
			?>
				<option value='0'><?php
					_e( 'Regular Front Page («Latest Posts» or «Static Page»)', 'nelio-ab-testing' ); ?></option>
				<option value='1'<?php
					if ( NelioABSettings::does_theme_use_a_custom_landing_page() )
						echo ' selected="selected"';
				?>><?php _e( 'Theme-based Front Page', 'nelio-ab-testing' ); ?></option>
			</select>
			<div class="the-descr" style="display:none;"><span class="description"><?php
