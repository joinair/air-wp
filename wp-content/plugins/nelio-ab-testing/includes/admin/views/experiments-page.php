	...
	class NelioABExperimentsPage extends NelioABAdminAjaxPage {
		...
		protected function do_render() {
			...
			if ( count( $this->experiments ) == 0 ) {
				...
				echo sprintf( '<img class="animated flipInY" src="%s" alt="%s" />',
					...
					__( 'Information Notice', 'nelio-ab-testing' )
				);
				echo '<h2 style="max-width:750px;">';
				printf( '%1$s<br><br><a class="button button-primary" href="%3$s">%2$s</a>',
					__( 'Find and manage all your experiments from this page.<br>Click the following button and create your first experiment!', 'nelio-ab-testing' ),
					_x( 'Create Experiment', 'create-experiment', 'nelio-ab-testing' ),
			...
			function nelioabValidateClick(msg_id, href) {
				...
				switch (msg_id) {
					case 0:<?php
						$title = __( 'Start Experiment', 'nelio-ab-testing' );
						$title = str_replace( '"', '\\"', $title );
						$msg = __( 'You are about to start an experiment. Once the experiment has started, you cannot edit it. Are you sure you want to start the experiment?', 'nelio-ab-testing' );
						$msg = str_replace( '"', '\\"', $msg ); ?>
						jQuery('#dialog-content').html("<?php echo $msg; ?>");
						$dialog.dialog('option', 'title', "<?php echo $title; ?>");
						$dialog.parent().find('.button-primary .ui-button-text').text("<?php _e( 'Start', 'nelio-ab-testing' ); ?>");
						$dialog.dialog('open');
						break;
					case 1:<?php
		...
		private function insert_duplicate_dialog() { ?>
			<div id="nelioab-scheduling-dialog" class="nelio-sect" title="<?php
				_e( 'Experiment Duplication', 'nelio-ab-testing' );
			?>">
				<p><?php _e( 'You are about to duplicate an experiment.<br>New name:', 'nelio-ab-testing' ); ?></p>
				...
				jQuery(function($) {<?php
					...
					var $dupDialog = $('#nelioab-scheduling-dialog').dialog({
						...
						buttons: [
							...
							{
								text: "<?php echo esc_html( __( 'Duplicate', 'nelio-ab-testing' ) ); ?>",
								'class': 'button button-primary',
								click: function() {
		...
		private function insert_schedule_dialog() { ?>
			<div id="nelioab-scheduling-dialog" title="<?php
				_e( 'Experiment Scheduling', 'nelio-ab-testing' );
			?>">
				<p><?php _e( 'Schedule experiment start for:', 'nelio-ab-testing' ); ?></p>
				<?php
				require_once( NELIOAB_UTILS_DIR . '/html-generator.php' );
				NelioABHtmlGenerator::print_scheduling_picker();
				?>
				<p class="error" style="color:red;display:none;"><?php
					_e( 'Please, specify a full date (month, day, and year) in the future.', 'nelio-ab-testing' );
				?></p>
			</div>
			<script>
				jQuery(function($) {<?php
					...
					$info.dialog({
						...
						buttons: [
							...
							{
								text: "<?php echo esc_html( __( 'Schedule', 'nelio-ab-testing' ) ); ?>",
								'class': 'button button-primary',
								click: function() {
									try {
		...
		function column_name( $exp ) {
			...
			switch( $exp->get_status() ) {
				...
				case NelioABExperiment::STATUS_READY:
					...
					$actions['start'] = sprintf( $url_dialog, 'start', $exp->get_id(), $exp->get_type(), __( 'Start', 'nelio-ab-testing' ), 0 );
					// $actions['schedule'] = sprintf( $url, 'schedule', $exp->get_id(), $exp->get_type(), __( 'Schedule' ) );
					$actions['duplicate'] = sprintf( $url_duplicate, 'duplicate', $exp->get_id(), $exp->get_type(), __( 'Duplicate' ), 'nelio-ab-testing' );
					$actions['trash'] = sprintf( $url, 'trash', $exp->get_id(), $exp->get_type(), __( 'Trash' ) );
					break;
				case NelioABExperiment::STATUS_SCHEDULED:
					$actions = array(
						'start'           => sprintf( $url_dialog, 'start', $exp->get_id(), $exp->get_type(), __( 'Start Now', 'nelio-ab-testing' ), 0 ),
						// 'schedule'        => sprintf( $url, 'schedule', $exp->get_id(), $exp->get_type(), __( 'Reschedule' ) ),
						// 'cancel-schedule' => sprintf( $url, 'cancel-schedule', $exp->get_id(), $exp->get_type(), __( 'Cancel Schedule', 'nelio-ab-testing' ), 1 ),
						'duplicate'       => sprintf( $url_duplicate, 'duplicate', $exp->get_id(), $exp->get_type(), __( 'Duplicate', 'nelio-ab-testing' ) ),
					);
					break;
				case NelioABExperiment::STATUS_RUNNING:
					$actions = array(
						'theprogress' => sprintf( $progress_url, $exp->get_id(), $exp->get_type(), __( 'View' ) ),
						'stop'        => sprintf( $url_dialog, 'stop', $exp->get_id(), $exp->get_type(), __( 'Stop', 'nelio-ab-testing' ), 1 ),
						'duplicate'   => sprintf( $url_duplicate, 'duplicate', $exp->get_id(), $exp->get_type(), __( 'Duplicate', 'nelio-ab-testing' ) ),
					);
					break;
			...
			if ( isset( $actions['start'] ) && $related_post && $related_post > 0 && get_post_status( $related_post ) !== 'publish' ) {
				$label = sprintf( '<span style="cursor:default;" title="%s">%s</span>',
					esc_html( __( 'The experiment cannot be started because the tested element has not been published yet', 'nelio-ab-testing' ) ),
					__( 'Start', 'nelio-ab-testing' ) );
				$actions['start'] = $label;
			}
			...
			if ( !NelioABAccountSettings::is_plan_at_least( NelioABAccountSettings::ENTERPRISE_SUBSCRIPTION_PLAN ) ) {
				...
				if ( isset( $actions['schedule'] ) )
					$actions['schedule'] = sprintf( '<span title="%s">%s</span>', $expl, __( 'Schedule', 'nelio-ab-testing' ) );
			}
			...
			if ( ! NelioABExperimentsManager::current_user_can( $exp ) ) {
				foreach ( $actions as $key => $value ) {
					if ( $key === 'edit' ) {
						...
						$actions[$key] = __( 'Start', 'nelio-ab-testing' );
					} else if ( $key === 'stop' ) {
						$actions[$key] = __( 'Stop', 'nelio-ab-testing' );
					} else if ( $key === 'schedule' ) {
						$actions[$key] = __( 'Schedule', 'nelio-ab-testing' );
					} else if ( $key === 'duplicate' ) {
						$actions[$key] = __( 'Duplicate', 'nelio-ab-testing' );
		...
		public function column_relevant_date( $exp ) {
			...
			case NelioABExperiment::STATUS_FINISHED:
					$res = sprintf( $date,
						...
						__( 'Finalization Date', 'nelio-ab-testing' ),
			...
			case NelioABExperiment::STATUS_RUNNING:
					$res = sprintf( $date,
						...
						__( 'Start Date', 'nelio-ab-testing' ),
			...
			case NelioABExperiment::STATUS_SCHEDULED:
					$res = sprintf( $date,
						strtotime( $exp->get_start_date() ),
						__( 'Scheduled Date', 'nelio-ab-testing' ),
						NelioABFormatter::format_date( $exp->get_start_date() ) );
					break;
				...
				default:
					$res = sprintf( $date,
						...
						__( 'Creation Date', 'nelio-ab-testing' ),
