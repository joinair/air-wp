	...
	class NelioABPostAltExpProgressPage extends NelioABAltExpProgressPage {
			...
			if ( $primary ) {
				if ( current_user_can( 'edit_post', $id ) ) {
					return sprintf( ' <a class="button button-primary" href="%1$s">%2$s</a>', $url,
						__( 'View Heatmap', 'nelio-ab-testing' ) );
				} else {
					...
					if ( $primary ) {
						if ( NelioABExperimentsManager::current_user_can( $exp, 'apply' ) ) {
							$aux = sprintf(
								...
								$alt_id, $img, __( 'Apply', 'nelio-ab-testing' ) );
						} else {
							$aux = sprintf( '<a class="apply-link button button-primary disabled" href="#">%s</a>', __( 'Apply', 'nelio-ab-testing' ) );
						}
					} else {
						if ( NelioABExperimentsManager::current_user_can( $exp, 'apply' ) ) {
							$aux = sprintf(
		...
		protected function print_the_original_alternative() {
			...
			$aux = sprintf(
				...
				esc_url( $link ), __( 'View Content', 'nelio-ab-testing' ) );
			...
			foreach ( $exp->get_alternatives() as $alt ) {
				...
				if ( current_user_can( 'edit_post', $alt->get_value() ) ) {
					...
					$aux = sprintf(
						...
						__( 'View Content', 'nelio-ab-testing' ) );
				}
				array_unshift( $action_links, $aux );
